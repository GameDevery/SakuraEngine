...
void RenderPassForward::execute(const skr_primitive_pass_context_t* context, skr::span<const skr_primitive_draw_packet_t> drawcalls)
{
    ...
    const skr::String ButtonText = skr::format(u8"SwitchShadingRate-{}", shadingRateNames[shading_rate]);
    ImGui::Begin("ShadingRate");
    if (ImGui::Button(ButtonText.data_raw()))
    {
    ...
    renderGraph->add_render_pass(
        ...
        [=, this](skr::render_graph::RenderGraph& g, skr::render_graph::RenderPassContext& pass_context) {
            ...
            cgpu_render_encoder_set_viewport(pass_context.encoder, 0.0f, 0.0f, (float)viewport->viewport_width, (float)viewport->viewport_height, 0.f, 1.f);
            cgpu_render_encoder_set_scissor(pass_context.encoder, 0, 0, viewport->viewport_width, viewport->viewport_height);

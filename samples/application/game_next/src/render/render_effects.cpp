...
#include "SkrBase/math/manual/transform.hpp"
...
skr_primitive_draw_packet_t RenderEffectForward::produce_draw_packets(const skr_primitive_draw_context_t* context)
{
    ...
    auto r_effect_callback = [&](sugoi_chunk_view_t* r_cv) {
        ...
        auto gBatchCallback = [&](sugoi_chunk_view_t* g_cv) {
            ...
            const auto l2ws         = sugoi::get_component_ro<skr::scene::TransformComponent>(g_cv);
            const auto translations = sugoi::get_component_ro<skr::scene::TranslationComponent>(g_cv);
            const auto rotations    = sugoi::get_component_ro<skr::scene::RotationComponent>(g_cv);
            (void)rotations;
            const auto scales = sugoi::get_component_ro<skr::scene::ScaleComponent>(g_cv);
...
#endif
                // FIXME: update parallel_for and transform
...
void RenderEffectForward::prepare_pipeline(SRendererId renderer)
{
    ...
    SKR_LOG_INFO(u8"Loading vertex shader: %s", vsname.u8_str());
    ...
    ppl_shaders[0].library              = _vs;
    ppl_shaders[1].stage                = CGPU_SHADER_STAGE_FRAG;
    ppl_shaders[1].entry                = SKR_UTF8("main");
    ppl_shaders[1].library              = _fs;

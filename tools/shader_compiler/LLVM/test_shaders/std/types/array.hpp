...
struct [[builtin("array")]] Array {
    ...
    constexpr void set(Type v) { set(start, v); }
    constexpr void set(uint32 loc, Type v) { access_(loc) = v; }
    [[access]] constexpr Type &access_(uint32 loc) { return v[loc]; }
    [[access]] constexpr Type &operator[](uint32 loc) { return v[loc]; }

    constexpr Type get(uint32 loc) const { return access_(loc); }
...
private:
    ...
    constexpr void set(Type v, Args... args) {
        set(start, v);
        set<start + 1>(args...);
    }
...
struct [[builtin("array")]] Array<int32, size, ArrayFlags::Shared> {
    ...
    template<uint32 start, typename... Args>
    constexpr void set(int32 v, Args... args) {
        set(start, v);
        set<start + 1>(args...);
    }
    template<uint32 start>
    constexpr void set(int32 v) { set(start, v); }
    constexpr void set(uint32 loc, int32 v) { access_(loc) = v; }
    [[access]] constexpr int32 &access_(uint32 loc) { return v[loc]; }
    [[access]] constexpr int32 &operator[](uint32 loc) { return v[loc]; }

    constexpr int32 get(uint32 loc) const { return access_(loc); }
...
struct [[builtin("array")]] Array<uint32, size, ArrayFlags::Shared> {
    ...
    template<uint32 start, typename... Args>
    constexpr void set(uint32 v, Args... args) {
        set(start, v);
        set<start + 1>(args...);
    }
    template<uint32 start>
    constexpr void set(uint32 v) { set(start, v); }
    constexpr void set(uint32 loc, uint32 v) { access_(loc) = v; }
    [[access]] constexpr uint32 &access_(uint32 loc) { return v[loc]; }
    [[access]] constexpr uint32 &operator[](uint32 loc) { return v[loc]; }

    constexpr uint32 get(uint32 loc) const { return access_(loc); }

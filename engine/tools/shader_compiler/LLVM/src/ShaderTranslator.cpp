...
const skr::CppSL::TypeDecl* FunctionStack::methodThisType() const
{
    if (auto method = llvm::dyn_cast<clang::CXXMethodDecl>(func))
    {
        ...
        {
            return pShaderTranslator->getType(method->getThisType()->getPointeeType());
        }
    }
    return nullptr;
}

inline void ShaderTranslator::ReportFatalError(const std::string& message) const
{
    llvm::report_fatal_error(message.c_str());
}

template <typename... Args>
inline void ShaderTranslator::ReportFatalError(std::format_string<Args...> _fmt, Args&&... args) const
{
...
template <typename... Args>
inline void ShaderTranslator::ReportFatalError(const clang::Stmt* expr, std::format_string<Args...> _fmt, Args&&... args) const
{
...
template <typename... Args>
inline void ShaderTranslator::ReportFatalError(const clang::Decl* decl, std::format_string<Args...> _fmt, Args&&... args) const
{
...
void ShaderTranslator::DumpWithLocation(const clang::Stmt* stmt) const
{
    stmt->getBeginLoc().dump(pASTContext->getSourceManager());
    stmt->dump();
}

void ShaderTranslator::DumpWithLocation(const clang::Decl* decl) const
{
...
std::unique_ptr<clang::ASTConsumer> CompileFrontendAction::CreateASTConsumer(clang::CompilerInstance& CI, llvm::StringRef InFile)
{
    ...
    return std::make_unique<skr::CppSL::ShaderTranslator>(AST);
}

ShaderTranslator::ShaderTranslator(skr::CppSL::AST& AST)
    : clang::ASTConsumer()
    , AST(AST)
{
...
FunctionStack* ShaderTranslator::zzNewStack(const clang::FunctionDecl* func)
{
...
bool ShaderTranslator::VisitNamespaceDecl(const clang::NamespaceDecl* namespaceDecl)
{
    TranslateNamespaceDecl(namespaceDecl);
    return true;
}

CppSL::TypeDecl* ShaderTranslator::TranslateEnumDecl(const clang::EnumDecl* enumDecl)
{
    using namespace clang;
...
CppSL::TypeDecl* ShaderTranslator::TranslateRecordDecl(const clang::RecordDecl* recordDecl)
{
    using namespace clang;
    ...
    if (BuiltinAttr != nullptr)
    {
        ...
        else if ((TSD && What == "texture2d") || (TSD && What == "texture3d") ||
            (TSD && What == "texture2d_array") || (TSD && What == "texture3d_array") ||
...
CppSL::NamespaceDecl* ShaderTranslator::TranslateNamespaceDecl(const clang::NamespaceDecl* namespaceDecl)
{
    using namespace clang;
...
const CppSL::TypeDecl* ShaderTranslator::TranslateLambda(const clang::LambdaExpr* x)
{
    if (!_lambda_proxy)
    {
...
void ShaderTranslator::TranslateLambdaCapturesToParams(const clang::LambdaExpr* expr)
{
...
CppSL::GlobalVarDecl* ShaderTranslator::TranslateGlobalVariable(const clang::VarDecl* Var)
{
...
CppSL::Stmt* ShaderTranslator::TranslateCall(const clang::Decl* _funcDecl, const clang::Stmt* callExpr)
{
...
bool ShaderTranslator::TranslateStageEntry(const clang::FunctionDecl* x)
{
...
bool ShaderTranslator::VisitFieldDecl(const clang::FieldDecl* x)
{
    if (IsDump(x))
        x->dump();
...
bool ShaderTranslator::VisitVarDecl(const clang::VarDecl* x)
{
    if (IsDump(x))
        x->dump();
    return true;
...
CppSL::TypeDecl* ShaderTranslator::TranslateType(clang::QualType type)
{
    type = Decay(type);
...
CppSL::ParamVarDecl* ShaderTranslator::TranslateParam(std::vector<CppSL::ParamVarDecl*>& params, skr::CppSL::EVariableQualifier qualifier, clang::QualType type, const skr::CppSL::Name& name)
{
...
void ShaderTranslator::TranslateParams(std::vector<CppSL::ParamVarDecl*>& params, const clang::FunctionDecl* func)
{
...
CppSL::FunctionDecl* ShaderTranslator::TranslateFunction(const clang::FunctionDecl* x, llvm::StringRef override_name)
{
    if (IsDump(x))
        x->dump();
...
template <typename T>
T* ShaderTranslator::TranslateStmt(const clang::Stmt* x)
{
    return (T*)TranslateStmt(x);
}

Stmt* ShaderTranslator::TranslateStmt(const clang::Stmt* x)
{
    using namespace clang;
    using namespace skr;
...
bool ShaderTranslator::addVar(const clang::VarDecl* var, skr::CppSL::VarDecl* _var)
{
...
skr::CppSL::VarDecl* ShaderTranslator::getVar(const clang::VarDecl* var) const
{
...
bool ShaderTranslator::addType(clang::QualType type, skr::CppSL::TypeDecl* decl)
{
    type = Decay(type);
...
bool ShaderTranslator::addType(clang::QualType type, const skr::CppSL::TypeDecl* decl)
{
    return addType(type, const_cast<skr::CppSL::TypeDecl*>(decl));
}

skr::CppSL::TypeDecl* ShaderTranslator::getType(clang::QualType type) const
{
    type = Decay(type);
    if (auto bt = type->getAs<clang::BuiltinType>())
    {
    ...
    else if (auto _array = pASTContext->getAsConstantArrayType(type))
    {
        ...
        {
            const_cast<ShaderTranslator*>(this)->TranslateType(ConstantArrayType->getElementType()->getCanonicalTypeInternal());
        }
    ...
    else if (auto _ptr = type->isPointerType())
    {
        ...
        {
            const_cast<ShaderTranslator*>(this)->TranslateType(type->getPointeeType()->getCanonicalTypeInternal());
        }
...
bool ShaderTranslator::addFunc(const clang::FunctionDecl* func, skr::CppSL::FunctionDecl* decl)
{
...
skr::CppSL::FunctionDecl* ShaderTranslator::getFunc(const clang::FunctionDecl* func) const
{
...
clang::QualType ShaderTranslator::Decay(clang::QualType type) const
{
...
void ShaderTranslator::CheckStageInputs(const clang::FunctionDecl* x, skr::CppSL::ShaderStage stage)
{
...
const clang::NamespaceDecl* ShaderTranslator::GetDeclNamespace(const clang::Decl* decl) const
{
    using namespace clang;
...
void ShaderTranslator::AssignDeclsToNamespaces()
{
    using namespace clang;

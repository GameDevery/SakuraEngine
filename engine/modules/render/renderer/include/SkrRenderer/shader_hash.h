#pragma once
#include "SkrRenderer/fwd_types.h"
#include "SkrGraphics/flags.h"
#ifdef __cplusplus
    #include "SkrContainers/string.hpp"
    #include "SkrRTTR/enum_tools.hpp"
#endif

#ifndef __meta__
    #include "SkrRenderer/shader_hash.generated.h" // IWYU pragma: export
#endif

sreflect_struct(
    guid = "5a54720c-34b2-444c-8e3a-5977c94136c3";
    serde = @bin | @json;)
SStableShaderHash
{
    uint32_t valuea = 0;
    uint32_t valueb = 0;
    uint32_t valuec = 0;
    uint32_t valued = 0;

#ifdef __cplusplus
    inline constexpr SStableShaderHash() = default;

    inline bool operator==(const SStableShaderHash& other) const
    {
        return valuea == other.valuea && valueb == other.valueb && valuec == other.valuec && valued == other.valued;
    }

    struct hasher
    {
        SKR_RENDERER_API size_t operator()(const SStableShaderHash& hash) const;
    };

    SKR_RENDERER_API static SStableShaderHash hash_string(const char* str, uint32_t size) SKR_NOEXCEPT;

    SKR_RENDERER_API static SStableShaderHash from_string(const char* str) SKR_NOEXCEPT;

    SKR_RENDERER_API SStableShaderHash(uint32_t a, uint32_t b, uint32_t c, uint32_t d) SKR_NOEXCEPT;
    SKR_RENDERER_API explicit operator skr::String() const SKR_NOEXCEPT;
#endif
};
static SKR_CONSTEXPR SStableShaderHash kZeroStableShaderHash = SStableShaderHash();

sreflect_struct(
    guid = "0291f512-747e-4b64-ba5c-5fdc412220a3";
    serde = @bin | @json;)
SPlatformShaderHash
{
    uint32_t flags;
    uint32_t encoded_digits[4];

#ifdef __cplusplus
    inline bool operator==(const SPlatformShaderHash& other) const
    {
        return encoded_digits[0] == other.encoded_digits[0] && encoded_digits[1] == other.encoded_digits[1] && encoded_digits[2] == other.encoded_digits[2] && encoded_digits[3] == other.encoded_digits[3] && flags == other.flags;
    }

    struct hasher
    {
        SKR_RENDERER_API size_t operator()(const SPlatformShaderHash& hash) const;
    };
#endif
};

sreflect_struct(
    guid = "b0b69898-166f-49de-a675-7b04405b98b1";
    serde = @bin | @json;)
SPlatformShaderIdentifier
{
#ifdef __cplusplus
    skr::EnumAsValue<ECGPUShaderBytecodeType> bytecode_type;
    skr::EnumAsValue<ECGPUShaderStage> shader_stage;
#else
    ECGPUShaderBytecodeType bytecode_type;
    ECGPUShaderStage shader_stage;
#endif
    SPlatformShaderHash hash;

#ifdef __cplusplus
    inline bool operator==(const SPlatformShaderIdentifier& other) const
    {
        return hash == other.hash && bytecode_type == other.bytecode_type && shader_stage == other.shader_stage;
    }

    struct hasher
    {
        SKR_RENDERER_API size_t operator()(const SPlatformShaderIdentifier& hash) const;
    };
#endif
};